...
//		console.log("fnIsScrollBottom", containerHeight, ' + ', containerScrollTop, ' = ', (containerHeight + containerScrollTop), ' > ', documentHeight, ' + ', scrollMargin, ' = ', (documentHeight - scrollMargin), lastPage);
	...
	var resizeCover = function() {
		...
		console.log("width:" + imgWidth + "px; height:" + imgHeight + "px;", 1000);
//		showSnackbar("width:" + imgWidth + "px; height:" + imgHeight + "px;", 1000);
	};

	var sortVideo = function(list, sort, reverse) {
	...
	var manipulateDom = function() {
		...
		$("#img-width").val(getLocalStorageItem(VIDEOLISTBYSPA_IMAGE_WIDTH, 290));
	};
	...
	var eventHandle = function() {
		...
		$(".btn-group-sort").children().on('click', function() {
			...
			$(this).parent().children().each(function() {
				var sort = $(this).data("sort");
				$(this).swapClass("btn-success", "btn-default", true).attr({title: sort.name}).html(sort.code).css({borderColor: "#3e8f3e"});
			});
			...
			sortVideo(videoList, sort.code, reverse);
		...
		$("#novideo").on("click", function() {
			...
			//console.log("#novideo click", isCheckedNoVideo);
			render(true);
		});
		...
		$("#img-width").on("change", resizeCover).trigger("change");
		
		$("#viewMore").on("click", render);
				
		$(window)
			.on("resize", resizeSecondDiv).trigger("resize")
			.on('keyup', function(e) {
