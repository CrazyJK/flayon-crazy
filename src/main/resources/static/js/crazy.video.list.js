...
//									$("<dd>").append(video.label_videoCandidates).append("&nbsp;").append(video.label_torrentSeed)
			...
			parent.append(
					$("<tr>", {
						...
						"mouseup": function(e) {
							...
							$("<td>").css({maxWidth: "100px"}).attr({title: video.actressName}).append(
									$('<div>').addClass("nowrap").append(
											video.label_actress
									)
							),
		...
		$(".status").html(renderingCount + " / " + (isFilter ? filteredCount : videoList.length));
...
//			console.log("tagKeys", tagKeys);
		...
		restCall(PATH + '/rest/video', {data : {t: withTorrent, p: 0}, title: "request video"}, function(list) {
			videoList = [];
